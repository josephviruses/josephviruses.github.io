...
<div align='center'>
![Table of results](/josephviruses.github.io/content/post/first_table.png)
</div>
```{r}
...
~~It shall be noted that the cutechess implementation by TCEC was not updated to properly convert Leela's evaluation into centipawns.~~ __It was actually the code for centipawn conversion that had to be updated.__[^5] The result was the very low centipawn evaluation scores shown by Lc0 even at more than 90% losing evaluation. This may account for the 7 losses from mates seen during the SuFi.

[^5]: https://github.com/LeelaChessZero/lc0/pull/1193
...
p <- sufi17 %>% 
  ggplot(aes(Lc0, SF, color = Result, group=game)) + geom_point() + geom_abline(data = data.frame(x=c(0,1.7), y = c(0,1.7)), alpha=0.5) +
...
<center>
```{r, fig.height=8, fig.width=6, fig.cap="Distribution of opening evaluations for different ECO codes"}
library(stickylabeller)
data_df %>% 
...
<center>
```{r, fig.cap="Distribution of number of moves by piece"}
...
<center>
```{r, fig.cap="Plot of Leela's evaluations against Stockfish's evaluations"}
...
```{r, echo=TRUE, fig.cap="Logistic fit over Leela and Stockfish's evaluations.", fig.align='center'}
...
```{r}
library(chron)
sufi17$GameDuration <- chron(times = sufi17$GameDuration)
```
All of the SuFi games went on for a total duration of `r sum(sufi17$GameDuration)` days (except for short intervals between games). The average time for a SuFi game is `r format(as.POSIXct(as.numeric(mean(sufi17$GameDuration))* 86400, origin = "1970-01-01", tz = "UTC"), "%H:%M")` with a standard deviation of `r format(as.POSIXct(as.numeric(chron(times = sd(sufi17$GameDuration)))*86400, origin="1970-01-01", tz="UTC"), "%H:%M")`.
...
sufi17 %>%
  mutate(ECO=str_extract(ECO, "[A-Z]+")) %>%
  group_by(ECO) %>%
  summarise(Duration = mean(GameDuration),
            SD = chron(times = sd(GameDuration)),
            Min = min(GameDuration),
            Q1 = quantile(GameDuration, .25),
            Median = median(GameDuration),
            Q3 = quantile(GameDuration, .75),
            Max = max(GameDuration)) %>%
  flextable() %>%
  autofit()
```
...
```{r}
sufi17 %>%
  filter(White == "LCZero v0.24-sv-t60-3010") %>%
  mutate(ECO=str_extract(ECO, "[A-Z]+")) %>%
  group_by(ECO) %>%
  summarise(Duration = mean(GameDuration),
            SD = chron(times = sd(GameDuration)),
...
```{r}
sufi17 %>%
  filter(White == "Stockfish 20200407DC") %>%
  mutate(ECO=str_extract(ECO, "[A-Z]+")) %>%
  group_by(ECO) %>%
  summarise(Duration = mean(GameDuration),
            SD = chron(times = sd(GameDuration)),
...
<center>
```{r, fig.height=5, fig.width=9, fig.cap="Outcome based on opening engine evaluations"}
p6 <- data_df %>%
  mutate(White = sufi17$White) %>%
  mutate(opening = rep(1:50, each = 2)) %>%
  group_by(opening) %>%
  mutate(paired_result = 
           case_when(first(Result) == "1/2-1/2" & last(Result) == "1/2-1/2"~"Both draws",
         first(Result) == "1-0" & last(Result) == "1-0"~"Both wins",
         TRUE~"One Win")) %>%
  ggplot(aes(Lc0, SF, color = paired_result, group=game)) + 
  geom_point() +
  geom_abline(data = data.frame(x=c(0,1.75),y=c(0,1.75)), alpha = 0.5) +
  facet_wrap(~White+Result, labeller = label_glue("({.l}) White = {White}\nResult={Result}")) 
p6
```
</center>
